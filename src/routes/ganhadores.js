    ...
    return (<PublicPage>
      <div className="mx-auto max-w-7xl px-4 py-12">
        ...
        {isLoading ? (<div className="space-y-4">
            {[1, 2, 3, 4, 5].map(i => (<div key={i} className="h-20 bg-surface animate-pulse rounded-2xl"/>))}
          </div>) : winners && winners.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {winners.map((winner) => {
                const profile = winner.profiles;
                const name = profile?.display_name || 'Ganhador';
                const abbreviatedName = name.split(' ').map((n, i) => i === 0 ? n : n[0] + '.').join(' ');
                return (<div key={winner.id} className="surface-card p-6 flex flex-col gap-6 hover-lift relative overflow-hidden group">
          ...
          </div>) : (<div className="text-center py-20 border-2 border-dashed border-border rounded-3xl">
