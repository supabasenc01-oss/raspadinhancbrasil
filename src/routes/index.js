...
import { Zap, Trophy, ArrowRight, Ticket, Clock, History } from 'lucide-react';
...
function HomePage() {
    ...
    return (<PublicPage>
      {/* Hero Section */}
      <PageHero title="ETAPA 5 — EXPERIÊNCIA VISUAL PREMIUM 2026" description="Aperfeiçoando a experiência visual com animações de ponta, motor de raspagem interativo e design ultra-moderno." centered>
      ...
      <section className="py-20 px-4">
        <div className="mx-auto max-w-7xl">
          ...
          {isLoading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((i) => (<div key={i} className="aspect-[3/4] rounded-3xl bg-surface animate-pulse"/>))}
            </div>) : scratchCards && scratchCards.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {scratchCards.map((card) => (<ScratchCardTile key={card.id} card={card}/>))}
            </div>) : (<div className="text-center py-20 surface-card">
      ...
      <section className="py-20 bg-primary/[0.02] border-y border-primary/5">
        ...
        ].map((step, i) => (<div key={i} className="flex flex-col items-center text-center space-y-4">
      ...
      <section className="py-20 px-4 overflow-hidden">
        ...
        ].map((winner, i) => (<div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-surface border border-border/50 hover:border-accent/30 transition-colors group">
